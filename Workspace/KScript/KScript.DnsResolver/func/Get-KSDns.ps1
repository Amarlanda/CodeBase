function Get-KSDns {
  ...
  # .DESCRIPTION
  #   Get-KSDns is a debugging resolver tool similar to dig and nslookup.
  # .PARAMETER DnsDebug
  #   Forces Get-KSDns to output intermediate requests which would normally be hidden, such as NXDomain replies when using a SearchList.
  # .PARAMETER DnsSec
  ...
  # .PARAMETER IPv6
  #   Force the use of IPv6 for queries, if this parameter is set and the Server is set to a name (e.g. ns1.domain.example), Get-KSDns will attempt to locate an AAAA record for the server.
  # .PARAMETER Iterative
  #   Perform a full iterative search for a name starting with the root servers. Intermediate queries are resolved using the locally configured name server to reduce the work-load as Get-KSDns does not implement response caching.
  # .PARAMETER Name
  #   A resource name to query, by default Get-KSDns will use '.' as the name. IP addresses (IPv4 and IPv6) are automatically converted into an appropriate format to aid PTR queries.
  # .PARAMETER NoEDns
  ...
  #   If a name is not root terminated (does not end with '.') a SearchList will be used for recursive queries. If this parameter is not defined Get-KSDns will attempt to retrieve a SearchList from the hosts network configuration.
  #
  #   SearchLists are explicitly dropped for Iterative, NSSearch, Zone Transfer and Version queries.
  ...
  # .PARAMETER Server
  #   A server name or IP address to execute a query against. If an IPv6 address is used Get-KSDns will attempt the query using IPv6 (enables the IPv6 parameter). 
  #
  #   If a name is used another lookup will be required to resolve the name to an IP. Get-KSDns caches responses for queries performed involving the Server parameter. The cache may be viewed and maintained using the *-InternalDnsCache CmdLets.
  #
  #   If no server name is defined, the Get-KSDnsServerList CmdLet is used to discover locally configured DNS servers.
  # .PARAMETER Tcp
  ...
  if ($NoEDns -and $DnsSec) {
    Write-Warning "Get-KSDns: EDNS support is mandatory for DNSSEC. Enabling EDNS for this request."
    $NoDns = $false
  }
  if ($NoEDns) {
  ...
  if ([IPAddress]::TryParse($Name, [Ref]$IPAddress)) {
    ...
    if ($IPAddress.AddressFamily -eq [Net.Sockets.AddressFamily]::InterNetwork) {
      ...
      $IPAddressString = ConvertTo-KSString $IPAddressBytes -Hexadecimal        
  ...
  if (-not ($psboundparameters.ContainsKey("Server"))) {
    if ($IPv6) {
      $Server = (Get-KSDnsServerList -IPv6 | Select-Object -ExpandProperty IPAddressToString | Select-Object -First 1)
    } else {
      $Server = (Get-KSDnsServerList | Select-Object -ExpandProperty IPAddressToString | Select-Object -First 1)
    }
  }
  if (-not $Server) {
  ...
  if ($IPv6) {
    $ServerRecordType = [KScript.DnsResolver.RecordType]::AAAA
  } else {
    $ServerRecordType = [KScript.DnsResolver.RecordType]::A
  }
  ...
  if ([IPAddress]::TryParse($Server, [Ref]$IPAddress)) {
    ...
    if ($IPAddress.AddressFamily -eq [Net.Sockets.AddressFamily]::InterNetworkv6) {
      Write-Verbose "Get-KSDns: Server: IPv6 Server value used. Switching to IPv6 transport."
      $IPv6 = $true
    }
  } else {
    ...
    $CachedServer = Get-KSInternalDnsCacheRecord $Server $ServerRecordType
    if ($CachedServer) {
      Write-Verbose "Get-KSDns: Cache: Using Server ($Server) from cache."
      ...
      Write-Verbose "Get-KSDns: Server: Attempting to lookup $Server $ServerRecordType"
      $DnsResponse = Get-KSDns $Server -RecordType $ServerRecordType
      ...
      if ($DnsResponse.Answer) {
        ...
        Write-Verbose "Get-KSDns: Cache: Adding Server ($Server) to cache."
        $DnsResponse.Answer | ForEach-Object {
          Add-KSInternalDnsCacheRecord -ResourceRecord $_
        }
      }
    }
    if ($IPAddress) {
  ...
  if ($NoSearchList -or $NSSearch -or $Iterative -or $ZoneTransfer -or $Name.EndsWith('.')) {
    ...
    if (-not $SearchList) {
      ...
      $SearchList = Get-KSCimInstance -ClassName Win32_NetworkAdapterConfiguration |
        Where-Object DNSDomainSuffixSearchOrder |
        Select-Object -ExpandProperty DNSDomainSuffixSearchOrder |
        ForEach-Object { "$_." }
      Write-Verbose "Get-KSDns: SearchList: Automatically retrieved and set to $SearchList"
    }
  ...
  if ($Version) {
    ...
    $Name = "version.bind."
    $RecordType = [KScript.DnsResolver.RecordType]::TXT
    $RecordClass = [KScript.DnsResolver.RecordClass]::CH
  }
  ...
  if ($Iterative) {
    ...
    $HintRecordSet = Get-KSInternalDnsCacheRecord -RecordType A -ResourceType Hint
    ...
    while ($NoError -and $NoAnswer) {
      ...
      $DnsResponse = Get-KSDns $Name -RecordType $RecordType -RecordClass $RecordClass -NoRecursion -Server $Server @GlobalOptions
      
      if ($DnsResponse.Header.RCode -ne [KScript.DnsResolver.RCode]::NoError)  {
        $NoError = $false
      } else {
        if ($DnsResponse.Header.ANCount -gt 0) {
          ...
          $Server = $DnsResponse.Additional |
            Where-Object { $_.Name -eq $Authority.Hostname -and $_.RecordType -eq [KScript.DnsResolver.RecordType]::A } |
            Select-Object -ExpandProperty IPAddress |
            Select-Object -First 1
          if ($Server) {
            Write-Verbose "Get-KSDns: Iterative query: Next name server IP: $Server"
          } else {
            $Server = $Authority[0].Hostname
            Write-Verbose "Get-KSDns: Iterative query: Next name server Name: $Server"
          }
        }
      }
  ...
  if ($NSSearch) {
    ...
    Write-Verbose "Get-KSDns: NSSearch: Finding start of authority."
    $DnsResponse = Get-KSDns $Name -RecordType SOA -Server $Server -NoSearchList
    if ($DnsDebug) {
      $DnsResponse
    }
    if ($DnsResponse.Header.RCode -eq [KScript.DnsResolver.RCode]::NoError -and $DnsResponse.Header.ANCount -gt 0) {
      $ZoneName = $DnsResponse.Answer | Where-Object RecordType -eq ([KScript.DnsResolver.RecordType]::SOA) | Select-Object -ExpandProperty Name
    } elseif ($DnsResponse.Header.RCode -eq [KScript.DnsResolver.RCode]::NoError -and $DnsResponse.Header.NSCount -gt 0) {
      $ZoneName = $DnsResponse.Authority | Where-Object RecordType -eq ([KScript.DnsResolver.RecordType]::SOA) | Select-Object -ExpandProperty Name
    }
    
    # Get the name servers for the zone
    Write-Verbose "Get-KSDns: NSSearch: Finding name servers for zone ($ZoneName)."
    $DnsResponse = Get-KSDns $ZoneName -RecordType NS -Server $Server -NoSearchList
    if ($DnsDebug) {
      $DnsDebug
    }
    $AuthoritativeServerList = $DnsResponse.Answer | Where-Object RecordType -eq ([KScript.DnsResolver.RecordType]::NS) | ForEach-Object {
      $NameServer = $_
      $NameServerIP = $DnsResponse.Additional |
        Where-Object { $_.Name -eq $NameServer.Hostname -and ($_.RecordType -eq [KScript.DnsResolver.RecordType]::A -or $_.RecordType -eq [KScript.DnsResolver.RecordType]::AAAA) } |
        Select-Object -ExpandProperty IPAddress 
      if ($NameServerIP) {
    ...
    Write-Verbose "Get-KSDns: NSSearch: Testing responses from each authoritative servers"
    $AuthoritativeServerList | ForEach-Object {
      Get-KSDns $Name -RecordType $RecordType -RecordClass $RecordClass -Server $_ -NoSearchList @GlobalOptions
    }
  }
  ...
  if ($RecordType -eq [KScript.DnsResolver.RecordType]::IXFR -and -not $SerialNumber) {
    # SerialNumber is required, throw an error and abandon this.
    $ErrorRecord = New-Object Management.Automation.ErrorRecord(
  ...
  if ($ZoneTransfer -and $psboundparameters.ContainsKey("SerialNumber")) {
    ...
    $RecordType = [KScript.DnsResolver.RecordType]::IXFR
    $DnsResponse = Get-KSDns $Name -RecordType $RecordType -Server $Server -SerialNumber $SerialNumber -NoSearchList
    
    if ($DnsResponse.Header.RCode -eq [KScript.DnsResolver.RCode]::NoError) {
      if ($DnsResponse.Answer[0].Serial -le $SerialNumber) {
        # Complete, the zone is already up to date.
        Write-Verbose "Get-KSDns: IXFR: Transfer complete, zone is up to date."
        $DnsResponse
      } else {
        if ($DnsResponse.Header.ANCount -eq 1 -and $DnsResponse.Answer[0].RecordType -eq [KScript.DnsResolver.RecordType]::SOA) {
          # The message was a UDP overflow response, restart using TCP
          Write-Verbose "Get-KSDns: IXFR: UDP overflow response. Attempting TCP."
          Get-KSDns $Name -RecordType $RecordType -Server $Server -SerialNumber $SerialNumber -Tcp -NoSearchList
        }
      }
    } else {
    ...
    $RecordType = [KScript.DnsResolver.RecordType]::AXFR
    $Tcp = $true
  ...
  if (-not $Iterative -and -not $NSSearch -and -not $ZoneTransfer) {
    $SearchStatus = [KScript.DnsResolver.RCode]::NXDomain; $i = 0
    ...
    do {
      ...
      Write-Verbose "Get-KSDns: Query: $FullName $RecordClass $RecordType :: Server: $Server Protocol: $(if ($Tcp) { 'TCP' } else { 'UDP' }) AddressFamily: $(if ($IPv6) { 'IPv6' } else { 'IPv4' })"

      # Construct a message
      if ($RecordType -eq [KScript.DnsResolver.RecordType]::IXFR -and $SerialNumber) {
        $DnsQuery = NewKSDnsMessage -Name $FullName -RecordType $RecordType -RecordClass $RecordClass -SerialNumber $SerialNumber
      } else {
        $DnsQuery = NewKSDnsMessage -Name $FullName -RecordType $RecordType -RecordClass $RecordClass
      }
      if (-not $NoEDns -and -not ($RecordType -in ([KScript.DnsResolver.RecordType]::AXFR), ([KScript.DnsResolver.RecordType]::IXFR))) {
        $DnsQuery.SetEDnsBufferSize($EDnsBufferSize) 
      }
      if ($DnsSec -and -not ($RecordType -in ([KScript.DnsResolver.RecordType]::AXFR), ([KScript.DnsResolver.RecordType]::IXFR))) {
        $DnsQuery.SetAcceptDnsSec()
      }
      
      if ($NoRecursion) {
        # Recursion is set by default, toggle the flag.
        $DnsQuery.Header.Flags = [KScript.DnsResolver.Flags]([UInt16]$DnsQuery.Header.Flags -bxor [UInt16][KScript.DnsResolver.Flags]::RD)
      }
      ...
      if ($Tcp -and $IPv6) {
        ...
        $Socket = New-KSSocket -SendTimeout $Timeout -ReceiveTimeout $Timeout -IPv6
        try {
          Connect-KSSocket $Socket -RemoteIPAddress $Server -RemotePort $Port
        ...
        Send-KSBytes $Socket -Data ($DnsQuery.ToByte([Net.Sockets.ProtocolType]::Tcp))
      
      } elseif ($Tcp) {
      
        # Create a TCP socket, connect and send the message.
        $Socket = New-KSSocket -SendTimeout $Timeout -ReceiveTimeout $Timeout
        try {
          Connect-KSSocket $Socket -RemoteIPAddress $Server -RemotePort $Port
        } catch [Net.Sockets.SocketException] {
          $ErrorRecord = New-Object Management.Automation.ErrorRecord(
        ...
        $Socket = New-KSSocket -ProtocolType Udp -SendTimeout $Timeout -ReceiveTimeout $Timeout -IPv6
        Send-KSBytes $Socket -RemoteIPAddress $Server -RemotePort $Port -Data ($DnsQuery.ToByte())
      
      } else {
      
        # Create a UDP socket and send the message.
        $Socket = New-KSSocket -ProtocolType Udp -SendTimeout $Timeout -ReceiveTimeout $Timeout
        Send-KSBytes $Socket -RemoteIPAddress $Server -RemotePort $Port -Data ($DnsQuery.ToByte())
      ...
      do {
        try {
          $DnsResponseData = Receive-KSBytes $Socket -BufferSize 4096
        } catch [Net.Sockets.SocketException] {
          $ErrorRecord = New-Object Management.Automation.ErrorRecord(
            (New-Object Net.Sockets.SocketException ($_.Exception.InnerException.NativeErrorCode)),
        ...
        if ($Tcp) {
          ...
          if ($MessageBuffer.Count -ge 2) {
            ...
            while ($MessageBuffer.Count -ge ($MessageLength + 2)) {
              ...
              $DnsResponse = ReadKSDnsMessage $DnsResponseData
              ...
              if ($RecordType -eq [KScript.DnsResolver.RecordType]::IXFR -and -not $MessageComplete) {
                if ($ActiveSerialNumber -eq $null) {
                  # A truncated return.
                  if ($DnsResponse.Header.ANCount -eq 1 -and $DnsResponse.Answer[0].RecordType -eq [KScript.DnsResolver.RecordType]::SOA) {
                    $MessageComplete = $true
                    Write-Verbose "Get-KSDns: IXFR: Terminated, no more answers available."
                  }
                  if ($DnsResponse.Header.ANCount -ge 2) {
                    if ($DnsResponse.Answer[1].RecordType -ne [KScript.DnsResolver.RecordType]::SOA) {
                      ...
                      $RecordType = [KScript.DnsResolver.RecordType]::AXFR
                      Write-Verbose "Get-KSDns: IXFR: AXFR mode response detected."
                    } else {
                      $ActiveSerialNumber = $DnsResponse.Answer[0].Serial
                      Write-Verbose "Get-KSDns: IXFR: Latest serial number available is $ActiveSerialNumber"
                    }
                  }
                }
                ...
                if ($ActiveSerialNumber) {
                  $SOAResouceRecordCount += $DnsResponse.Answer |
                    Where-Object { $_.RecordType -eq [KScript.DnsResolver.RecordType]::SOA -and $_.Serial -eq $ActiveSerialNumber } |
                    Measure-Object |
                    Select-Object -ExpandProperty Count
               
                  if ($SOAResouceRecordCount -ge 3) {
                    $MessageComplete = $true
                    Write-Verbose "Get-KSDns: IXFR: Transfer is complete."
                  }
                }
              }
              
              # AXFR completion tests
              if ($RecordType -eq [KScript.DnsResolver.RecordType]::AXFR -and -not $MessageComplete) {
                $SOAResouceRecordCount += $DnsResponse.Answer |
                  Where-Object RecordType -eq ([KScript.DnsResolver.RecordType]::SOA) |
                  Measure-Object |
                  Select-Object -ExpandProperty Count
                ...
                if ($SOAResouceRecordCount -ge 2) {
                  $MessageComplete = $true
                  Write-Verbose "Get-KSDns: AXFR: Transfer is complete."
                }
              } else {
                # If this is not a zone transfer the process can be marked as complete.
                $MessageComplete = $true
                Write-Verbose "Get-KSDns: Query: Complete."
              }
            }
          }
        } else {
        ...
        if ($DnsResponse) {
          ...
          # Return the response
          if ($SearchStatus -ne [KScript.DnsResolver.RCode]::NXDomain -or $DnsDebug) {
            if ($KSDnsCacheReverse.Contains($DnsResponse.Server)) {
              $DnsResponse.Server = "$($DnsResponse.Server) ($($KSDnsCacheReverse[$DnsResponse.Server]))"
            }
       
            if ($DnsResponse.Header.Flags -band [KScript.DnsResolver.Flags]::TC) {
              if ($NoTcpFallback) {
                $DnsResponse
              } else {
                ...
                $DnsResponse = $null
                # Resend using TCP
                Get-KSDns @psboundparameters -Tcp
              }
            } else {
              $DnsResponse
            }
        ...
        if ($SearchStatus -eq [KScript.DnsResolver.RCode]::NXDomain -and $i -eq ($SearchList.Count - 1)) {
          if ($RecordType -in ([KScript.DnsResolver.RecordType]::AXFR), ([KScript.DnsResolver.RecordType]::IXFR)) {
            Write-Warning "Get-KSDns: Transfer refused. Server ($Server) is not authoritative for $Name."
          } else {
            Write-Warning "Get-KSDns: Name ($Name) does not exist."
          }
        }
      ...
      if ($Tcp) {
        ...
        Disconnect-KSSocket $Socket
      }
      # Close down the socket and free resources.
      Remove-KSSocket $Socket

      # Track the position in the suffixes search list
      $i++

    } while ($SearchStatus -eq [KScript.DnsResolver.RCode]::NXDomain -and $i -lt $SearchList.Count)
  }
}
