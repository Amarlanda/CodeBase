function Get-KSLocalGroup {
  # .SYNOPSIS
  #   Gets local groups from a computer.
  # .DESCRIPTION
  #   Get-KSLocalGroup returns all local user accounts on a machine, including all properties exposed by the WinNT provider. 
  ...
  # .EXAMPLE
  #   Get-LocalGroup | Select-Object SystemName, Name, Class, Description
  #
  #   Get users from the local computer.
  # .EXAMPLE
  #   Get-Content ServerList.txt | ForEach-Object { Get-LocalGroup -ComputerName $_ }
  #
  #   Get local users from each.
  # .EXAMPLE
  #   Get-KSADComputer -OperatingSystem "Windows 7*" | Get-LocalGroup
  #
  ...
  process {
    ...
    if ($IsDomainController) {
      ...
      $DirectoryEntry.PSBase.Children | Where-Object { $_.Class -eq "group" -and $_.Name -like $Name } | ForEach-Object {
        $Object = ConvertFromKSADPropertyCollection $_.Properties -ObjectPath $_.Path -ObjectType $_.Class
        $Object.PSObject.TypeNames.Add("KScript.Local.Group")
