function ConvertFrom-KSXPathNode {
  # .SYNOPSIS
  #   Convert from an XPathDocumentNavigator node-set to the specified type.
  # .DESCRIPTION
  #   ConvertFrom-KSXPathNode simplifies reading XML documents using the XPathNavigator. The node-set in the input pipeline is converted to the requested type.
  ...
  #   System.String
  # .EXAMPLE
  #   $XPathNavigator.Select("/Node") | ConvertFrom-KSXPathNode -ToString
  # .NOTES
  #   Author: Chris Dent
  ...
  process {
    switch ($pscmdlet.ParameterSetName) {
      ...
      'ToHashtable' {
        ...
        if ($Values.Count -eq 1) {
          if ($UseTypeAttribute) {
            ...
            $Value = ConvertTo-KSType $Values.Value $Type
          } else {
            $Value = ConvertTo-KSType $Values.Value
          }
          $Hashtable.Add($Key, $Value)
        } else {
          if ($UseTypeAttribute) {
            $Value = $XmlNode | ConvertFrom-KSXPathNode -ToObject -UseTypeAttribute
          } else {
            $Value = $XmlNode | ConvertFrom-KSXPathNode -ToObject
          }
          $Hashtable.Add($Key, $Value)
        }
      }
          ...
          if ($UseTypeAttribute) {
            ...
            if ($Type -match '\[\]$') {
              $Value = $_.Select("./*") | ForEach-Object {
                ConvertTo-KSType $_.Value ($Type -replace '\[\]$')
              }
            } else {
              $Value = ConvertTo-KSType $Value $Type
            }
          } else {
            $Value = ConvertTo-KSType $Value
          }
      ...
      'ToString' {
        return (ConvertTo-KSType $XmlNode.Value.Trim())
      }
    }
  }
